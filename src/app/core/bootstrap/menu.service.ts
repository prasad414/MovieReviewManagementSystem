...
export class MenuService {
  ...
  getAll(): Observable<Menu[]> {
    // const allMenus = ["dashboard", "design", "material", "media", "forms", "tables", "profile", "sessions", "helpers"];
    const allowedMenus = ['movies']; // Only show allowed menus
    //const allowedMenus = []; // Means Show all menus
    let tmpMenu = this.menu$.value;
    // tmpMenu = tmpMenu.filter(x => allowedMenus.length == 0 || allowedMenus.indexOf(x.route) >= 0);
